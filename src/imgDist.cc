...
vector<Patch> ImageDistCalculator::get_patches(const Img& img) const {
	int noww = 0, nowh = 0;
	vector<Patch> ret;
	while (nowh <= img.h - self->PATCH_SIZE) {
		while (noww <= img.w - self->PATCH_SIZE) {
			ret.emplace_back(noww, nowh);
			noww += self->PATCH_SHIFT;
		}
		nowh += self->PATCH_SHIFT;
		noww = 0;
	}
	return ret;
}
...
real_t ImageDist::cal_IMED(const Img& img1, Patch p1, const Img& img2, Patch p2, real_t thres) const {
	thres = thres * thres;
...
real_t ImageDistCalculator::cal_dIE(const vector<Patch>& p1, const vector<Patch>& p2) const {
	...
	REP(i, m.h)  {
		REP(j, m.w) {
			real_t ret = self->cal_IMED(img2, p2[i], img1, p1[j], max(row_min[i], col_min[j]));
			m.get(i, j) = ret;
...
real_t ImageDistCalculator::calculate() {
	auto p1 = get_patches(img1),
		 p2 = get_patches(img2);
	return cal_dIE(p1, p2);
}
...
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < n; i ++) {
		...
		real_t dist = ImageDistCalculator(this, orig, results[i]).calculate();
		PP(dist);
#pragma omp critical
